...
void draw() {
  ...
  //animate panels with mouse location
  if (mouseY < pHeight) {
  ...
  if (button==1) {
    ...
    float x = width/2; //sets ellipse to center
    float y = height - x;
    float eWidth = width - 50; //sets padding around target


    for (float spacing = random (10, 80); eWidth > 0; eWidth = eWidth - 2*spacing) { //double spacing to acount for both ellipses
